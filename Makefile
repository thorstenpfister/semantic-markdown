...
# Generate Homebrew formula from local build (for testing only)
homebrew-formula: release-checksums ## Generate Homebrew formula file (LOCAL BUILD - use homebrew-formula-github for releases)
	@echo "WARNING: Generating formula from LOCAL build"
	@echo "For production releases, use 'make homebrew-formula-github' after 'make github-release'"
	@echo ""
	...
	@echo ""
	@echo "WARNING: This formula uses LOCAL build checksums"
...
homebrew-formula-github: ## Generate Homebrew formula from GitHub release (optionally with TAP_DIR=/path/to/tap)
	...
	@echo "Downloading release artifacts from GitHub to verify checksums..."
	@mkdir -p $(DIST_DIR)/github-download
	@echo "Fetching darwin-arm64..."
	@curl -sL "https://github.com/thorstenpfister/semantic-markdown/releases/download/$(VERSION)/semantic-md-$(VERSION)-darwin-arm64.tar.gz" \
		-o "$(DIST_DIR)/github-download/semantic-md-$(VERSION)-darwin-arm64.tar.gz" || \
		(echo "Error: Failed to download darwin-arm64 from GitHub. Has the release been created?"; exit 1)
	@echo "Fetching darwin-amd64..."
	@curl -sL "https://github.com/thorstenpfister/semantic-markdown/releases/download/$(VERSION)/semantic-md-$(VERSION)-darwin-amd64.tar.gz" \
		-o "$(DIST_DIR)/github-download/semantic-md-$(VERSION)-darwin-amd64.tar.gz"
	@echo "Fetching linux-arm64..."
	@curl -sL "https://github.com/thorstenpfister/semantic-markdown/releases/download/$(VERSION)/semantic-md-$(VERSION)-linux-arm64.tar.gz" \
		-o "$(DIST_DIR)/github-download/semantic-md-$(VERSION)-linux-arm64.tar.gz"
	@echo "Fetching linux-amd64..."
	@curl -sL "https://github.com/thorstenpfister/semantic-markdown/releases/download/$(VERSION)/semantic-md-$(VERSION)-linux-amd64.tar.gz" \
		-o "$(DIST_DIR)/github-download/semantic-md-$(VERSION)-linux-amd64.tar.gz"
	@echo ""
	@echo "Calculating checksums from GitHub artifacts..."
	@cd $(DIST_DIR)/github-download && shasum -a 256 *.tar.gz > checksums-github.txt
	@echo "Checksums from GitHub:"
	@cat $(DIST_DIR)/github-download/checksums-github.txt
	@echo ""
	@echo "Generating Homebrew formula..."
	@mkdir -p $(DIST_DIR)/homebrew
	@AMD64_SHA256=$$(grep "darwin-amd64.tar.gz" $(DIST_DIR)/github-download/checksums-github.txt | awk '{print $$1}'); \
	ARM64_SHA256=$$(grep "darwin-arm64.tar.gz" $(DIST_DIR)/github-download/checksums-github.txt | awk '{print $$1}'); \
	LINUX_AMD64_SHA256=$$(grep "linux-amd64.tar.gz" $(DIST_DIR)/github-download/checksums-github.txt | awk '{print $$1}'); \
	LINUX_ARM64_SHA256=$$(grep "linux-arm64.tar.gz" $(DIST_DIR)/github-download/checksums-github.txt | awk '{print $$1}'); \
	echo "# typed: false" > $(DIST_DIR)/homebrew/semantic-md.rb; \
	echo "# frozen_string_literal: true" >> $(DIST_DIR)/homebrew/semantic-md.rb; \
	echo "" >> $(DIST_DIR)/homebrew/semantic-md.rb; \
	echo "# This file was generated by semantic-markdown's Makefile from GitHub release $(VERSION)" >> $(DIST_DIR)/homebrew/semantic-md.rb; \
	echo "class SemanticMd < Formula" >> $(DIST_DIR)/homebrew/semantic-md.rb; \
	echo "  desc \"Convert HTML to clean, semantic Markdown optimized for LLMs\"" >> $(DIST_DIR)/homebrew/semantic-md.rb; \
	echo "  homepage \"https://github.com/thorstenpfister/semantic-markdown\"" >> $(DIST_DIR)/homebrew/semantic-md.rb; \
	echo "  version \"$(VERSION:v%=%)\"" >> $(DIST_DIR)/homebrew/semantic-md.rb; \
...
release: pre-release release-validate release-notes ## Run complete release workflow (builds and validates)
	@echo ""
	@echo "=========================================="
	...
	@echo "2. Create GitHub release: make github-release"
	@echo "3. Generate Homebrew formula: make homebrew-formula-github"
	@echo "4. Update Homebrew tap: make homebrew-update TAP_DIR=/path/to/homebrew-tap"
	@echo "5. Push Homebrew tap changes"
	@echo ""
